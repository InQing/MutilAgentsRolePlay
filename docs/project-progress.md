...
- 2026-04-05：继续收紧表达层 prompt，新增按群聊 / 私聊 / 动态区分的动作约束与角色状态提示，明确要求模型只输出最终正文，不附带角色名、标题、引导语或解释腔。
- 2026-04-05：表达层新增 LLM 输出清洗，当前会统一去除代码块包裹、通用 meta 前缀、角色名前缀与外围引号；相关表达层测试已扩展到 `46` 项后端测试，当前全部通过。
- 2026-04-05：表达层现已补充角色自身关系网摘要输入；executor 会在生成文本前注入该角色“自己指向他人的关系边”，prompt 可读取目标关系亲疏、affinity 与最近关系标签，但不会暴露其他角色彼此之间的关系。
- 2026-04-05：新增关系网进入表达层的回归测试，覆盖 prompt 中的关系摘要展示与 executor 只传入角色自身关系边的约束；本地轻量模式下后端测试扩展到 `47` 项，当前全部通过。

## 下一步

- 当前最优先项为：
  - 继续阶段四：基于已落地的 provider 通道和新 prompt 约束，接入实际模型配置并验证真实输出质量
  - 保持 `LLMClient` 兼容边界不变，避免把表达层锁死到单一供应商
  - 在不改 runtime 主骨架的前提下，继续提升角色表达质量、prompt 管理、输出清洗与可测试性
- 表达层稳定后，按以下顺序继续开发：
