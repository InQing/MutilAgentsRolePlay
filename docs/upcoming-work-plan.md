...
1. 先做导演模式的延迟可见规则，让更多敏感事件类型进入统一可见性判断
2. 再补导演页前端回归检查，确保控制与注入入口可持续校验
3. 导演能力稳定后，转入表达层升级
4. 最后再推进基础设施深化
...
1. 继续扩展延迟可见规则，把更多敏感事件类型纳入统一的可见性判断
2. 为导演页补前端交互回归检查，确保控制入口、互动展示与 inject event 在 Node 环境下可持续校验
3. 导演能力稳定后，切到表达层升级
4. 表达层稳定后，再回到 Redis / runtime 模式深化
