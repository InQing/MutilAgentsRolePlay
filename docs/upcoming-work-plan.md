...
1. 进入阶段三，先做表达层抽象，把文本生成从 `AutonomousActionExecutor` 中拆出去
2. 再把角色 persona / emotion_state / current_plan_summary / recent context 接到表达层输入
3. 然后接入真实 LLM 表达生成，但保持现有 runtime / scheduler / social 主链路不变
4. 表达层稳定后，再回补导演模式的细化可见性规则与前端回归检查
...
- 因此，导演能力当前更适合作为“细则收尾项”，主开发焦点可切到阶段三表达层升级

当前最建议的新窗口接手顺序：

1. 提取独立表达层或 message generation 层
2. 让角色状态接入表达层，拉开同一动作在不同角色上的表达差异
3. 通过 `LLMClient` 接入真实模型，但保持表达层可 mock、可测试
4. 表达层稳定后，再回补导演细则与 Redis / runtime 深化
